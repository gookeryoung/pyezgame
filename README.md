...
pyezgame 将 [GameLib](https://github.com/skywind3000/GameLib) 这个面向 C++ 初学者的游戏开发库移植到了 Python。通过 pybind11 封装，Python 开发者可以用简洁的 Python 语法实现窗口创建、图形绘制、精灵动画、声音播放、键盘鼠标输入等功能，无需了解底层 C++ 细节。
...
| `15_ui_controls.py` | UI 控件演示 | button、checkbox、radio_box、slider、spinner、tab_panel、dropdown、knob 等 |
...
| `knob(x, y, size, value, min_val, max_val)` | 旋钮控件，垂直拖拽调节，返回 `(changed, value)` |
...
| `tab_panel(x, y, w, h, tabs, selected_tab)` | 标签面板，返回 `(active_tab, cx, cy, cw, ch)` 含内容区域坐标 |
| `collapsible(x, y, w, title, open)` | 可折叠区域标题，返回 `(triggered, open)` |
| `menu(x, y, items, open)` | 弹出菜单，返回 `(selected_index, open)`，选中或点击外部自动关闭 |

#### 其他

| 方法 | 说明 |
|-|-|
| `tooltip(x, y, text)` | 工具提示浮层 |
| `image_button(x, y, w, h, sprite_id, color)` | 图片按钮（用精灵代替文字），点击返回 `True` |

> **提示**：`tab_panel` 返回的 `(cx, cy, cw, ch)` 是面板内部内容区域的坐标和尺寸，可直接用于放置子控件。
