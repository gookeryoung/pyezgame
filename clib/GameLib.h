...
bool GameLib::Slider(int x, int y, int w, int *value, int minVal, int maxVal)
{
    ...
    // draw handle
    bool handleHovered = PointInRect(_mouseX, _mouseY, handleX, y, handleW, handleH);
...
bool GameLib::Dropdown(int x, int y, int w, const char **items, int itemCount, int *selectedIndex, bool *open)
{
    ...
    int triCy = y + h / 2;
    // draw triangle: down when closed, up when open
...
int GameLib::TabPanel(int x, int y, int w, int h, const char **tabs, int tabCount, int selectedTab)
{
    ...
    // erase bottom border of active tab to connect with content
    {
